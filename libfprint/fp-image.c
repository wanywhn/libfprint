...
static void fp_image_sfm_extract_free(ExtractSfmData* data)
{
    g_clear_pointer(&data->image, g_free);
...
static void fp_image_sfm_extract_cb(GObject* source_object, GAsyncResult* res,
                                    gpointer user_data)
{
...
static void fp_image_sfm_extract_thread_func(GTask* task, void* src_obj,
                                             void* task_data,
...
SfmImgInfo* fp_image_get_sfm_info(FpImage* self) { return self->sfm_info; }

void fp_image_extract_sfm_info(FpImage* self, GCancellable* cancellable,
