...
typedef void (*SynCmdMsgCallback)(FpiDeviceElanmoc *self, uint8_t *buffer_in,
...
static uint8_t *elanmoc_compose_cmd(const struct elanmoc_cmd *cmd_info) {
...
static void elanmoc_cmd_ack_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                               gsize length_in, GError *error) {
  if (error) {
...
static void fp_cmd_receive_cb(FpiUsbTransfer *transfer, FpDevice *device,
...
static void fp_cmd_run_state(FpiSsm *ssm, FpDevice *dev) {
...
static void fp_cmd_ssm_done(FpiSsm *ssm, FpDevice *dev, GError *error) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(dev);
  CommandData *data = fpi_ssm_get_data(ssm);
...
static void fp_cmd_ssm_done_data_free(CommandData *data) { g_free(data); }

static void elanmoc_get_cmd(FpDevice *device, guint8 *buffer_out,
  ...
  self->cmd_ssm =
      fpi_ssm_new(FP_DEVICE(self), fp_cmd_run_state, FP_CMD_NUM_STATES);
...
static void enroll_status_report(FpiDeviceElanmoc *self, int enroll_status_id,
...
static void elanmoc_get_enrolled_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                                    gsize length_in, GError *error) {
  if (error) {
    fpi_ssm_mark_failed(self->task_ssm, error);
    return;
...
static void elanmoc_reenroll_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                                gsize length_in, GError *error) {
  if (error) {
    fpi_ssm_mark_failed(self->task_ssm, error);
    return;
  ...
  if (buffer_in[0] != 0x40) {
    ...
    if ((self->curr_enrolled == (ELAN_MAX_ENROLL_NUM + 1)) &&
        (buffer_in[1] == 0x00)) {
...
static void elanmoc_enroll_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                              gsize length_in, GError *error) {
  if (error) {
    fpi_ssm_mark_failed(self->task_ssm, error);
    return;
...
static void elanmoc_commit_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                              gsize length_in, GError *error) {
  if (error) {
    fpi_ssm_mark_failed(self->task_ssm, error);
    return;
...
static void elan_enroll_run_state(FpiSsm *ssm, FpDevice *dev) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(dev);
  guint8 *cmd_buf = NULL;
  guint8 *data = NULL;
  ...
  case MOC_ENROLL_REENROLL_CHECK:
    ...
    elanmoc_get_cmd(dev, cmd_buf, elanmoc_check_reenroll_cmd.cmd_len,
                    elanmoc_check_reenroll_cmd.resp_len, 0,
                    elanmoc_reenroll_cb);
    break;
...
static void task_ssm_done(FpiSsm *ssm, FpDevice *dev, GError *error) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(dev);
...
static FpPrint *create_print_from_response(FpiDeviceElanmoc *self,
  ...
  if (buffer_in[1] != ELAN_MSG_OK) {
    g_propagate_error(error, fpi_device_error_new_msg(
                                 ...
                                 "Device returned error %d rather than print!",
                                 buffer_in[1]));
    return NULL;
  }
...
static void elanmoc_get_userid_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                                  gsize length_in, GError *error) {
  FpPrint *print;
...
static void elan_list_run_state(FpiSsm *ssm, FpDevice *dev) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(dev);
  guint8 *cmd_buf = NULL;
  ...
  case MOC_LIST_GET_FINGER:
    ...
    elanmoc_get_cmd(dev, cmd_buf, elanmoc_get_userid_cmd.cmd_len,
                    elanmoc_get_userid_cmd.resp_len, 0, elanmoc_get_userid_cb);
    break;
  }
}

static void elanmoc_list(FpDevice *device) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(device);
...
static void elanmoc_match_report_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                                    gsize length_in, GError *error) {
  FpDevice *device = FP_DEVICE(self);
  FpPrint *print = NULL;
  ...
  fp_info("Verify/Identify successful for: %s",
...
static void identify_status_report(FpiDeviceElanmoc *self, int verify_status_id,
                                   int data, GError *error) {
  FpDevice *device = FP_DEVICE(self);
  guint8 *cmd_buf = NULL;
  ...
  case RSP_VERIFY_OK: {
    ...
    cmd_buf = elanmoc_compose_cmd(&elanmoc_get_userid_cmd);
    cmd_buf[2] = data;
    elanmoc_get_cmd(device, cmd_buf, elanmoc_get_userid_cmd.cmd_len,
                    elanmoc_get_userid_cmd.resp_len, 0,
...
static void elanmoc_identify_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                                gsize length_in, GError *error) {
  if (error) {
    fpi_ssm_mark_failed(self->task_ssm, error);
    return;
...
static void elan_identify_run_state(FpiSsm *ssm, FpDevice *dev) {
  guint8 *cmd_buf = NULL;
...
static void elanmoc_enroll(FpDevice *device) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(device);
  FpPrint *print = NULL;
  GVariant *data = NULL;
  GVariant *uid = NULL;
...
static void elanmoc_delete_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                              gsize length_in, GError *error) {
  if (error) {
    fpi_ssm_mark_failed(self->task_ssm, error);
    return;
...
static void elan_delete_run_state(FpiSsm *ssm, FpDevice *dev) {
  guint8 *cmd_buf = NULL;
...
static void elanmoc_delete_print(FpDevice *device) {
...
static void elanmoc_identify(FpDevice *device) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(device);
...
static void task_ssm_init_done(FpiSsm *ssm, FpDevice *dev, GError *error) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(dev);
...
static void elanmoc_cmd_dim_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                               gsize length_in, GError *error) {
  if (error) {
    fpi_ssm_mark_failed(self->task_ssm, error);
    return;
...
static void elanmoc_get_status_cb(FpiDeviceElanmoc *self, uint8_t *buffer_in,
                                  gsize length_in, GError *error) {
  guint8 *cmd_buf = NULL;
...
static void dev_init_handler(FpiSsm *ssm, FpDevice *dev) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(dev);
  guint8 *cmd_buf = NULL;
...
static void elanmoc_open(FpDevice *device) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(device);
  GError *error = NULL;
  gint productid = 0;
...
static void task_ssm_exit_done(FpiSsm *ssm, FpDevice *dev, GError *error) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(dev);
...
static void dev_exit_handler(FpiSsm *ssm, FpDevice *dev) {
  guint8 *cmd_buf = NULL;

  switch (fpi_ssm_get_cur_state(ssm)) {
  case DEV_EXIT_ABOVE:
    cmd_buf = elanmoc_compose_cmd(&elanmoc_above_cmd);
    elanmoc_get_cmd(dev, cmd_buf, elanmoc_above_cmd.cmd_len,
                    elanmoc_above_cmd.resp_len, 0, elanmoc_cmd_ack_cb);
    break;
  }
}

static void elanmoc_close(FpDevice *device) {
  FpiDeviceElanmoc *self = FPI_DEVICE_ELANMOC(device);
...
static void fpi_device_elanmoc_init(FpiDeviceElanmoc *self) { G_DEBUG_HERE(); }

static void fpi_device_elanmoc_class_init(FpiDeviceElanmocClass *klass) {
